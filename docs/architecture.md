...
If the consumer needs `IEnumerable<MediaFrame>` for some reason, there's an `[Obsolete]` `ConvertEnumerable(src, dst)` shim on `Swscale`.
...
- C# disallows `await` inside `unsafe` methods. The async demuxer extensions mark only their inner helpers as unsafe and keep the awaiting method outside.

## Where to put what

| Question                                          | Place it in                                                                 |
|---------------------------------------------------|-----------------------------------------------------------------------------|
| New raw FFmpeg field accessor                     | `src/Internal/<wrapper>.cs` or `src/MediaData/<wrapper>.Pascal.cs` (typed)   |
...
| New async / cancellation surface                  | `src/<subsystem>/<wrapper>.Async.cs`                                        |
| New high-level pipeline class (MediaSink-style)   | `src/<subsystem>/` as its own file                                          |
